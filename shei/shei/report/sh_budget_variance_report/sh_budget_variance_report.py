	...
	for year in fiscal_year:
		for from_date, to_date in get_period_date_ranges(filters["period"], year[0]):
			if filters["period"] == "Yearly":
				...
				for label in labels:
					columns.append(label+":Currency/currency:150")
			else:
				for label in [_("Budget") + " (%s)" + " " + str(year[0]), _("Actual") + " (%s)" + " " + str(year[0]), _("Variance") + " (%s)" + " " + str(year[0])]:
					...
					columns.append(label+":Currency/currency:150")
	if filters["period"] != "Yearly" :
		return columns + [_("Total Budget") + ":Currency/currency:150", _("Total Actual") + ":Currency/currency:150",
			_("Total Variance") + ":Currency/currency:150"]
	else:
		return columns
