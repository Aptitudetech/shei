					...
					for month in months:
						for year in years:
							...
							if 'total' in data[i]:
								...
								if col:
									columns.remove(col[0])
	# We might want to add data from Budget Variance report
	set_data_columns_for_budget_variance(filters, columns, data, month_list)
	return columns, data, None, chart


def set_data_columns_for_budget_variance(filters, columns, data, month_list):
	if filters.periodicity == 'Yearly':
		set_data_for_bv_yearly(filters, data, columns)
	elif filters.only in month_list:
		get_budget_variance_for_given_month(filters, data, columns)
	elif filters.only in ['1', '2', '3', '4']:
		get_quarterly_budget_variance(filters, data, columns)


def set_columns_filters_for_budget_variance(filters):
...
def set_data_for_bv_yearly(filters, data, columns):
	columns += set_columns_filters_for_budget_variance(filters)
	start_year = get_fiscal_year(date=filters.from_fiscal_year.split('-')[1], company=filters.company,
								 as_dict=True)
	year = start_year['year_end_date'].year
	month_abbr = calendar.month_name[start_year['year_end_date'].month][:3].lower()
	year_key = "{month_abbr}_{year}".format(month_abbr=month_abbr, year=year)
...
def get_quarterly_budget_variance(filters, data, columns):
	columns += set_columns_filters_for_budget_variance(filters)
	bv_data = bv_get_data(filters)
	...
	# [[budget nov, actual nov, variance nov], [budget dec, actual dec, variance dec]]
	# This calculation also remove the first 2 data (Cost center and Account), if applicable, because they
...
def get_budget_variance_for_given_month(filters, data, columns):
	columns += set_columns_filters_for_budget_variance(filters)
