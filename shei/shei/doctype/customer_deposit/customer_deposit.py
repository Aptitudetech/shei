...
class CustomerDeposit(Document):
    ...
    def update_posting_date(self):
        count = 0
        for i in frappe.get_list("Customer Deposit",
                                 fields=["name", "customer_deposit_application", "final_invoice_date"],
                                 filters={"docstatus": 1}):
            if i.final_invoice_date:
                frappe.db.set_value("Journal Entry", i.customer_deposit_application, "posting_date",
    ...
    def get_customer_deposit_quotation(self):
        ...
        for qt in frappe.get_list("Quotation", fields=["name", "total", "grand_total", "status"],
                                  filters={"customer_deposit": True, "customer_deposit_received": False, "docstatus": 1,
    ...
    def apply_customer_deposit(self):
        ...
        if not self.customer_deposit_application:
            ...
            accounts = frappe.get_all('Party Account', fields=['*'],
                                      filters={'parenttype': 'Customer', 'parent': self.customer})
            if accounts:
                # take the account from the table
                rec_account = accounts[0]['account']
            else:
                # take default receivable account
                rec_account = frappe.db.get_value('Company', {'name': frappe.db.get_default("company")},
                                                  'default_receivable_account')
            deposit_account = frappe.db.get_value('Bank Account',
                                                  {'currency': customer_currency, 'is_deposit_account': True},
                                                  'deposit_account')
    ...
    def check_if_multi_currency(self, currency):
        if currency == "USD":
            return True
        elif currency == "CAD":
            return False
    ...
    def on_submit(self):
        ...
        for i in self.get('customer_deposit_quotation'):
            ...
            je.append("accounts", {
                "account": bank_account_info['deposit_account'],
                "balance": 0,
                "cost_center": customer_deposit_defaults['default_cost_center'],
                "party_type": "Customer",
                "party": self.customer,
                "party_balance": 0,
            ...
            je.append("accounts", {
                "account": bank_account_info['bank_account'],
                "balance": 0,
                "cost_center": customer_deposit_defaults['default_cost_center'],
                "account_currency": qt.currency,
            ...
            for t in qt.get('taxes'):
                ...
                je.append("accounts", {
                    "account": customer_deposit_defaults['account_for_sales_tax'],
                    "balance": 0,
                    "cost_center": customer_deposit_defaults['default_cost_center'],
                    "account_currency": qt.currency,
                    "debit_in_account_currency": t.tax_amount,
